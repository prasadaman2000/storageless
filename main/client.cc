...
Client::Client(std::string ip_addr, int connectport) {
  ...
  if (int result = inet_pton(AF_INET, ip_addr.c_str(), &(servaddr.sin_addr));
      result <= 0) {
    std::cout << "inet_pton failed: " << result << "\n";
  }
  ...
  while (retry_count < kMaxNumRetries) {
    ...
    if (connect(connfd, (struct sockaddr*)&servaddr, sizeof(servaddr)) < 0) {
      std::cerr << "WARNING: could not connect to " << ip_addr << ":"
                << connectport << " will retry (errno: " << errno << ")\n";
      sleep(10);
      ++retry_count;
    } else {
      break;
  ...
  if (retry_count == kMaxNumRetries) {
    std::cerr << "FATAL: could not connect to " << ip_addr << ":" << connectport
              << ". Exiting...\n";
    exit(1);
  }
  ...
  worker_thread_ = std::thread([this]() { listen(); });
}
